...
Dictionaries, or Dicts, use something called key-value pairs to store data. A key is a unique piece of data in the dictionary that is assigned to a value. You cannot have duplicate keys, but you can have duplicate values. Dictionaries use `{}` instead of `[]` or `()`, and each key value pair is denoted by `key:value`. Each pair is separated by a comma.
