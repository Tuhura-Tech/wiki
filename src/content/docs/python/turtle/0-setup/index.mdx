...
title: Python Turtle Setup
description: This is how to setup the Turtle extension in python.
---
...
Make a new file, but do not name it turtle.py.
<details>
  <summary>Why does naming the file turtle.py fail?</summary>
  When you write `import turtle` inside the file (which is the next step), Python looks for a module called turtle in order to import it.
  This means the file imports itself, instead of importing the actual Turtle
  library.
</details>

## Import turtle library
When first starting a new file, the turtle library must be called.
There are two options for calling the turtle library.
