...
:::note
`len()` is a very useful method. It allows you to get the length of, or the number of items in, a collection. You can also use it on other things such as `str` to get the number of characters in the string.
:::
...
# This will get the item called "cherry" from the set
cherry = "cherry" in str_set

# This will remove the given item from the set, and return it.
removed_value = str_set.discard("tomato")

# This will add the item called "apple" to the set
str_set.add("apple")
...
# This will print each item in the set. Because it is unordered it might print in a different order each time.
for item in str_set:
    print(item)
