...
When working with 2D Space, we work on two axis, X and Y. When working in a 3D space, we add a third, Z. In Godot, Y represents up and down, while X and Y represent the two horizontal axis.

While mousing over the 3D game viewport, holding **Right Click** will allow to fly around the viewport using **WASD**. You can use the **scroll wheel** to control your speed. While not holding **Right Click** you can use the **scroll wheel** to zoom in and out.
...
So if we accessed and modified 'position' we might get some unexpected results. Wheras 'global_position' is relative to the entire scenes origin (0,0,0) and allows us to more reliably understand where a node is.
...
When deleting an object from our scene, we have two options, the **queue_free()** function, and the **free()** function, but what's the difference? **queue_free()** schedules a node to be deleted at the end of a frame. Wheras **free()** deletes it instantly.
...
This likely would make no difference at all to our project, but it's a good thing to keep in mind as you move onto bigger things!
...
Our second export will be our reference to our enemy. When we declare a variable in Godot, we can add ':'' followed by a variable type to allow only that type. This is especially useful for exports. (If not declaring a default value, exports require a type be given)
...
2. Move your spawner from the scene origin to somewhere else in the scene, so that it doesn't spawn things ontop of the objective.

After those steps, we should be good to test! Run your game, and let a few enemies spawn. 

Congratulations! Your game now has continuosly spawning enemes! Now to destroy them! 
...
Which as you may be able to guess, will mean we'll need to add something new to our Inputs. Incase you've forgotten, in the top left, click **Project -> Project Settings -> Input Map**
...
The -20 here determines the range at which our Raycast3D will detect colissions, you're welcome to adjust this value to be higher or lower to what feels right for your game.
...
Step 2 and 3 are easy enough, we've done both of those before. To check if the raycast is colliding with something, we'll use:
...
4. A label which says if you've won or lost.
...
These will all ensure the reticule is in the exact centre of the screen.
...
We'll add three simples functions which update these. Because these are so simple, we won't bother going over them line by line.
...
for a function that looks like this altogether:
...
However, we don't really need our timer to have its own scene, as we'll only have one in our scene. This makes this an excellent time to talk about **Autoload** scripts! Which are Godot's equivilent to Singletons if you're familiar from other Languages.
...
Our other two variables will be assigned in our **_ready()** function, which we'll setup now:
...
Finally, we need to update the text on our **time_label** to reflect the amount of time is left on our timer. Thankfully, Godot makes this easy for us. We'll do this directly in the **_process(delta)** function.
...
```
This trim's any characters after the 5th (Which keeps two decimal places) and then updates the text.
...
1. Modify the level to include walls to prevent the player falling, as well as platforms to jump on.
...
If you want to look over my version of the project, you can acess the Github repository here: [Link](https://github.com/lillianhidet/THT3DGame)
...
From here, you can make the project your own! Add your own models, music, textures. Add whole new mechanics! Look into gamefeel concepts and 'juice' the game up!
