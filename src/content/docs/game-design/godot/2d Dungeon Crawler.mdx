...
It'll need: a root **Rigidbody2D** (Called "Enemy") with three children: a **CollisionShape2D,** an **Area2D** (named "range") and an **AnimatedSprite2D**

Additionally we'll want to give the **Area2D** a **CollisionShape2D** as a child.

[TODO: Enemy scene image]

First, in the **Inspector** of the **Rigidbody2D** we'll want to set the **Gravity Scale** to 0 (As we don't want it to have gravity) and under the **Collision** section, we'll want **Layer** **2** and **3** selected. With only **1** selected under **Mask**
