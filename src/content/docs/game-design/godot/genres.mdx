...
Go to your player scene and add the root node to the `player` group by going to the node tab next to the inspector, going to groups, and adding `player.`

Then, in your singleton add an onready variable that holds the player. In a ready function on your autoload script find the player node with get_tree().get_first_node_in_group("player").
...
In theory, this should work. Of course, we don't have anything to tell us how much health the player has so let's quickly make a health bar.
