...
Simplex noise is a gradient noise algorithm that creates smooth and continuous variations across space. In this case:
 ...
 - Smoothness: Simplex noise ensures that nearby points in the 3D space have similar noise values, creating a naturally smooth progression for the road.
 - Fractal Detail: By using multiple octaves (fractal_octaves), the algorithm overlays layers of noise with increasing frequencies and decreasing amplitudes.
    This adds finer details to the base noise, making the road curves more realistic.
 - Directional Influence: The noise is sampled in specific ways (e.g., with offsets in x, y, or z) to introduce controlled randomness. For instance:
...
Increase the Noise Multiplier:
 - Try setting noise.get_noise_3d(...) * 150 or higher to see exaggerated, crazier turns.
Add Random Tilt:
 - You can add slight randomness to the Y-axis noise to tilt the road slightly.
Frequency Tuning:
 - Play with values between 0.001 and 0.01 for noise frequency to balance sharpness and smoothness.
