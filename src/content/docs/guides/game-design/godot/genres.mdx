...
It's good practice to separate functions into as many files as you can. This is helpful for keeping tidy, working with people using Git, help remind future-you what now-you was thinking, and just generally be easier. It's for this reason we'll use a short and separate script for each of the buttons. And just so we don't clutter up your scripts folder, we'll use built-in scripts which are stored in the `.tscn` file for your scene instead of a seperate `.gd` file.
...
Now, in your script get all four option nodes in your script. A good way to do this is to click and drag your node onto the script, and before you let go on an empty line press **Ctrl**. This will make the entire variable for you, instead of just dragging in `$"path/to/variable"`. You can even use **Ctrl** or **Shift** to select all four nodes at once, then drag, press control, and release to get all four variables at once.

Next, connect the `item_selected(index: int)` and the `toggled(toggled_on: bool` signals from all four option nodes.
...
@onready var msaa_2d = $MSAA2D
@onready var msaa_3d = $MSAA3D
@onready var fxaa = $FXAA
@onready var taa = $TAA
